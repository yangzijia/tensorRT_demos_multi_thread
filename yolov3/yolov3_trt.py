...
class Yolov3TRT(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.preprocessor = PreprocessYOLO(self.model_image_size)
        self.postprocessor = PostprocessYOLO(**self._get_postprocessor_args())
        self.engine = self.get_engine()
        self._tmp_buffers = self._allocate_buffers()

    def _get_postprocessor_args(self):
        return {
            "category": len(self.class_names),
            "yolo_masks": self.model_masks,
            "yolo_anchors": self.anchors,
            "obj_threshold": self.score,
            "nms_threshold": self.nms_threshold,
            "yolo_input_resolution": self.model_image_size
        }
    ...
    def get_engine(self):
        if os.path.exists(self.model_path):
            ...
            print("no {}".format(self.model_path))
            exit(0)

    def predict(self, matrix):
        self.get_detect_bboxes(matrix)

    def get_detect_bboxes(self, matrix):
        inputs, outputs, bindings, stream = self._tmp_buffers
        with self.engine.create_execution_context() as context:
            shape_orig_WH, image = self.preprocessor.my_process(matrix)
            w, h = shape_orig_WH
            inputs[0].host = image
            ...
            bboxes, categories, confidences = self.postprocessor.process(trt_outputs, shape_orig_WH)
            for box, score, category in zip(bboxes, confidences, categories):
                predicted_class = self.class_names[category]

                x_coord, y_coord, width, height = box
                left = max(0, np.floor(x_coord + 0.5).astype(int))
                top = max(0, np.floor(y_coord + 0.5).astype(int))
                right = min(w, np.floor(x_coord + width + 0.5).astype(int))
                bottom = min(h, np.floor(y_coord + height + 0.5).astype(int))
...
class PreprocessYOLO(object):
    ...
    def my_process(self, matrix):
        shape_orig_WH, image_resized = self.my_load_and_resize(matrix)
        image_preprocessed = self._shuffle_and_normalize(image_resized)
        return shape_orig_WH, image_preprocessed
    ...
    def my_load_and_resize(self, matrix):
        ...
        # image_raw = Image.fromarray(matrix)
        image_raw = Image.fromarray(cv2.cvtColor(matrix, cv2.COLOR_BGR2RGB))
        ...
        return image_raw.size, image_resized
